	...
	public void create() {
		...
		textureBuffer = BufferUtils.createFloatBuffer(12 * walls.size());
		{
			for(Wall w : walls) {
				...
				vertexBuffer.put(w.start.x);	vertexBuffer.put(w.start.y);				vertexBuffer.put(w.start.z);	//x y z
				vertexBuffer.put(w.end.x);		vertexBuffer.put(w.end.y);					vertexBuffer.put(w.end.z); 	//x y z
				vertexBuffer.put(w.end.x);		vertexBuffer.put(w.end.y);					vertexBuffer.put(w.end.z); 	//x y z 
